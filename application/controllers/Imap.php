...
class Imap {
    ...
    public function __construct($mailbox, $username, $password, $encryption = false) {
        ...
        if($encryption!=null && isset($encryption) && $encryption=='ssl') {
        		//if($mailbox!="imap.mail.me.com:993")        		
            		$enc = '/ssl';
        }
        else if($encryption!=null && isset($encryption) && $encryption=='tls')
            $enc = '/tls';
        $this->mailbox = "{" . $mailbox . $enc . "}";
        //var_dump($this->mailbox); exit;
        if($mailbox!="imap.mail.me.com:993/imap")
        		$this->imap = @imap_open($this->mailbox, $username, $password);
        else
        		$this->imap = @imap_open($this->mailbox, $username, $password,null, 1, array('DISABLE_AUTHENTICATOR'=>'PLAIN'));
    }
    ...
    public function searchMessages() {
    		...
    		$limit_date = strval(intval(date('Y'))-1).'-01-01';
    		$condition = ' SINCE "'.date('d-M-Y',strtotime($limit_date)).'"';
    		$email_ids = array_reverse(imap_search($this->imap, $condition), true);    		
    ...
    public function getMessages($withbody = true) {
        ...
        //for($i=$count;$i>=$count-4;$i--) {
            $emails = $this->formatMessage($count, $withbody);
            //if(strtotime($email['date']) < strtotime($limit_date))
//            		break;
            //$emails[]=$email;
        //}
